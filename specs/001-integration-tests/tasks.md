...
- [x] T107 [P] 创建 `tests/integration/docs/adding-tests.md`：详细的中文指南，说明如何添加新测试用例（<10分钟）
- [x] T108 [P] 创建 `tests/integration/docs/troubleshooting.md`：常见问题和解决方案

### 质量保证（章程原则I）

- [x] T109 运行完整测试套件验证：在本地环境运行 `./run-tests.sh --all`，确认所有测试通过
- [x] T110 验证测试覆盖范围：确认覆盖10+种输入法格式，30+测试用例（基于真实的单元测试文件）
- [x] T111 验证性能目标：确认完整测试套件在5分钟内完成
- [ ] T112 验证错误检测能力：确认能检测并报告至少95%的已知转换Bug

### 代码质量（Shell脚本最佳实践）

- [x] T113 [P] 代码审查和清理：确保所有Shell脚本符合Google Shell Style Guide
- [x] T114 [P] 添加或完善中文注释：确保所有函数、关键逻辑都有清晰的中文注释
- [x] T115 [P] 使用 shellcheck 进行静态分析：修复所有警告和错误
- [x] T116 [P] 优化脚本性能：减少不必要的子进程调用、优化循环逻辑

### 跨平台验证（章程原则IV）

- [ ] T117 在 Windows 上测试：使用 Git Bash 运行完整测试套件，验证所有功能正常
- [x] T118 在 Linux 上测试：在 Ubuntu 20.04+ 上运行完整测试套件
- [ ] T119 在 macOS 上测试：在 macOS 10.15+ 上运行完整测试套件
- [x] T120 验证路径处理：确保相对路径在所有平台上正确工作
- [x] T121 验证编码处理：确保 UTF-8、GBK、UTF-16 编码在所有平台上正确转换
